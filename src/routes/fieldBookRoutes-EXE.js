...
router.post('/', async (req, res) => {
		try {
			...
			if (fieldsQuery.length > 0) {
				...
				// Create the main Fields entry
				const AddedNewField = await prisma.fields.create({
					data: {
						...dataOnly_for_fields,
						district_id: Districts.id,
						tehsil_id: Tehsils.id,
						state_id: States.id,
						farmerSawie_nr: Farmer.sawie_nr
					}
				});
				
				console.log('New Field Added:', AddedNewField);
				
				// Create related entities if they exist
				if (Farmer && !Farmer.id) {
					const AddedNewFarmer = await prisma.farmer.create({
						data: {
							...Farmer,
							farmer_contact_person_id: 1, // Assuming static value
							super_visor_id: 1 // Assuming static value
						}
					});
					
					console.log('New Farmer Added:', AddedNewFarmer);
				}
				
				if (preparation_of_field.length > 0 && !preparation_of_field.id) {
					const AddedNewPreparationOfField = await prisma.preparationOfField.create({
						data: {
							...preparation_of_field,
							field_id: AddedNewField.id
							
						}
					});
					
					console.log('New Preparation of Field Added:', AddedNewPreparationOfField);
				}
